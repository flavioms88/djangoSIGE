...
from djangosige.apps.cadastro.models import Pessoa, Cliente, Fornecedor, Transportadora, Produto, Prospect
...
class InfoProspect(View):

    def post(self, request, *args, **kwargs):
        obj_list = []
        prospect = Prospect.objects.get(pk=request.POST['pessoaId'])
        obj_list.append(prospect)

        data = serializers.serialize('json', obj_list, fields=('nome_completo', 'cpf',))
